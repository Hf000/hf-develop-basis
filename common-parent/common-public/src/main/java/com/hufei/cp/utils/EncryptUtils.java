...
public class EncryptUtils {
    ...
    public static byte[] aesPublic(String encodeRules, byte[] byteContent, int cipherInt) throws Exception {
        ...
        KeyGenerator keygen = KeyGenerator.getInstance(CpConstant.AES);
        //2.根据ecnodeRules规则初始化密钥生成器    生成一个128位的随机源,根据传入的字节数组
        keygen.init(CpConstant.NUMBER_128, new SecureRandom(encodeRules.getBytes()));
        //3.产生原始对称密钥
        SecretKey originalKey = keygen.generateKey();
        //4.获得原始对称密钥的字节数组
        byte [] raw = originalKey.getEncoded();
        //5.根据字节数组生成AES密钥
        SecretKey key = new SecretKeySpec(raw, CpConstant.AES);
        //6.根据指定算法AES生成密码器
        Cipher cipher = Cipher.getInstance(CpConstant.AES);
