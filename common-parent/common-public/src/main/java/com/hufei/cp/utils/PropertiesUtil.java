...
//                properties.load(is);
    ...
    public String getPropertiesValue(String key) {
        if (StringUtils.startsWith(key, CpConstant.DOLLAR_SIGN_BEFORE_BIG_PARANTHESES)
                || StringUtils.endsWith(key, CpConstant.AFTER_BIG_PARANTHESES)) {
            key = StringUtils.replace(key, CpConstant.DOLLAR_SIGN_BEFORE_BIG_PARANTHESES, "");
            key = StringUtils.replace(key, CpConstant.AFTER_BIG_PARANTHESES, "");
        }
